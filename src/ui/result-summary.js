...
import { calculateStat, calculateStatBoost, getTypeEffectiveness, effectivenessInfo } from '../engine/stats.js';
...
function turnOrder(state) {
  if (!state.attacker.name || !state.defender.name) return { label: 'Awaiting Speed', tone: 'slate' };
  ...
  return { label: `${subjectName} moves ${first ? '1st' : '2nd'}`, tone: first ? 'emerald' : 'red' };
}
...
export function buildResultModel(rolls, state = STATE) {
  ...
  if (!state.attacker.name || !state.defender.name) {
    model = {
      ...
      moveText: 'Select both slots to calculate', moveTone: 'slate',
      pct: '0.0% - 0.0%',
    ...
    model = {
      ...
      moveText: line.text, moveTone: line.tone,
      pct: `${minPct.toFixed(1)}% - ${maxPct.toFixed(1)}%`,
      verdict,
      minFill: Math.min(100, minPct), maxFill: Math.min(100, maxPct),
    };
  }

  // Mode-driven identity icon + roll-gauge tier (shared across both views).
  model.iconGlyph = survival ? 'fa-shield-halved' : 'fa-hand-fist';
  model.iconTone = survival ? 'bg-blue-950/40 border-blue-900/40 text-blue-400'
    : 'bg-amber-950/40 border-amber-900/40 text-amber-400';
  model.gaugeTier = model.maxFill >= 100 ? 'bg-gradient-to-r from-red-600 to-rose-600'
