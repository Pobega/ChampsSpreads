...
// Mirror of updateRegulationTag (app.js): legality badge for the active format.
function regulationTag(apiName) {
  if (!apiName) return null;
  const reg = REGULATIONS[STATE.format];
  if (!reg) {
    return { text: 'National Dex', cls: 'bg-slate-800/60 text-slate-400 border border-slate-700/30 border' };
  }
